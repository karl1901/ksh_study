...
let baseUrl = {
  huhuiyu: 'https://service.huhuiyu.top'
};

server.huhuiyuBaseUrl = baseUrl.huhuiyu;

server.tokenkey = 'token';

server.huhuiyuLocalTokenKey = 'huhuiyuLocalToken';

server.getToken = function () {
  return localStorage.getItem(server.huhuiyuLocalTokenKey);
};

server.saveToken = function (info) {
  if (info) {
    localStorage.setItem(server.huhuiyuLocalTokenKey, info);
    return;
  }
...
server.getDownLoadUrl = function (id) {
  return (
    server.huhuiyuBaseUrl +
    '/teach_project_service/user/file/download' +
    '?' +
    'fid=' +
    id
  );
